...
from tools.terminal_tool_guards import (
    ...
    gateway_lifecycle_block, link_guard_block, self_repo_block,
)
...
def _pre_exec_block(
    ...
    then the dangerous-workdir check, then the link guard (symlink/junction
    creation + recursive deletes that escape the work tree through a link),
    then the self-repo guard (local only).
    """
